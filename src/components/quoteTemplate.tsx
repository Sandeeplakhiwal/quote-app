import { Quote } from "@/app/quotes/quotesRendrer";
import React, { useEffect, useState } from "react";

function QuoteTemplate({ id, text, author }: Quote) {
  const [isLiked, setIsLiked] = useState(false);
  const [isSaved, setIsSaved] = useState(false);

  useEffect(() => {
    let storedLikedArr = localStorage.getItem("likedQuotes");
    let likedArr: string[] = JSON.parse(storedLikedArr || "[]");
    let quoteIndexInLikedArr = likedArr.findIndex((i) => i === String(id));
    if (quoteIndexInLikedArr !== -1) {
      setIsLiked(true);
    }
  }, [id]);
  useEffect(() => {
    let storedSavedArr = localStorage.getItem("savedQuotes");
    let savedArr: string[] = JSON.parse(storedSavedArr || "[]");
    let quotesIndexInSavedArr = savedArr.findIndex((i) => i === String(id));
    if (quotesIndexInSavedArr !== -1) {
      setIsSaved(true);
    }
  }, [id]);
  ...
  const likeHandler = () => {
    if (window !== undefined) {
      ...
      let quoteIndexInLikedArr = likedArr.findIndex((i) => i === String(id));
      if (quoteIndexInLikedArr !== -1) {
        likedArr.splice(quoteIndexInLikedArr, 1);
        setIsLiked(false);
      } else {
        likedArr.push(String(id));
        setIsLiked(true);
      }
      localStorage.setItem("likedQuotes", JSON.stringify(likedArr));
    }
  };
  ...
  const saveHandler = () => {
    if (window !== undefined) {
      let storedSavedArr = localStorage.getItem("savedQuotes");
      let savedArr: string[] = JSON.parse(storedSavedArr || "[]");
      let quoteIndexInSavedArr = savedArr.findIndex((i) => i === String(id));
      if (quoteIndexInSavedArr !== -1) {
        savedArr.splice(quoteIndexInSavedArr, 1);
        setIsSaved(false);
      } else {
        savedArr.push(String(id));
        setIsSaved(true);
      }
      localStorage.setItem("savedQuotes", JSON.stringify(savedArr));
    }
  };
    ...
    <div className="w-auto border border-gray-200 mb-4 my-1 px-2 py-2 rounded-md max-w-[320px] sm:max-w-screen-lg mx-auto">
      ...
      <div className="w-auto rounded-lg">
        <p className=" font-normal text-xs pl-2">
          <span className=" font-bold text-xs">Author:-</span> {author}
        </p>
